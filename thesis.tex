...
We will discuss several such non-interactive proofs in the following sections. In each case, we also provide an explicit verification algorithm; a careful specification of theirs is important to avoid issues discussed in~\cite{mcmurtry2020test}. We do not have room to discuss formal proofs of their properties, but refer the interested reader to~\cite{boneh2020graduate} which covers the ideas in great detail. We will assume the presence of a hash function $\mathsf{Hash}$ that accepts inputs from any set and outputs an integer.
...
\begin{algorithm}\caption{Verification for~\ref{prot:PEP}}\label{prot:VerifyPEP}
    ...
    \begin{algorithmic}[1]
        \State $m \gets e'/a'$
        \State $c \gets \mathsf{Hash}(\mathbb{G}, g, q, y, d, e, d', e', a_1, b_2, a_2, b_2, a', \mathsf{IsEq}, t_1, t_2, t_1', t_2')$
        \If{$d^z = t_1\cdot d'^c$, $e^z = t_2\cdot e'^c$, $d'^{z'} = t_1\cdot a'^c$, $g^{z'} =t_2 y^c$ and not $d' = e' = 1$}
