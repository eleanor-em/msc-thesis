...
Building on the above ideas, our proposed system solves the auditing problem while achieving a degree of coercion resistance. Our system easily achieves cast-as-intended verifiability because it produces a paper ballot. It achieves tallied-as-cast verifiability first by relying on scrutineers inspecting ballots that arrive, and then by a system of cryptographic proofs. While our system is not end-to-end verifiable, it is verifiable under the loose condition that either the voter's client or the electoral commission (including the postal channel) is honest.
...
\begin{definition}[The ElGamal cryptosystem]\label{def-elgamal}
    The generation algorithm $\Gen(\lambda)$ for security parameter $\lambda\in\mathbb{N}$ runs as follows: let $\mathbb{G}$ be a cyclic group of order $q$ and $g\in\mathbb{G}$ be a generator, where $q$ is a $\lambda$-bit prime. Choose $x\in\mathbb{Z}_q$ uniformly at random, and let $y=g^x$. The public key of the scheme is $(\mathbb{G}, g, q, x)$ and the secret key is $(\mathbb{G}, g, q, x)$.
...
\begin{definition}[EC Verifiability]
    ...
    \begin{enumerate}%make this i and ii
