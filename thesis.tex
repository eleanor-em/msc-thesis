...
We first discuss how Percy can prove that he knows a secret \(s\) such that \(g^s=h\), and use this to prove plaintext knowledge for an ElGamal ciphertext. We will expand this to proving particular decryptions afterwards, following the treatment of \cite{damgaard2002sigma}.

Suppose Percy has published an element \(g\in\mathbb{Z}^\times_p\) of order \(q\) and \(h\in\mathbb{Z}^\times_q\). Valerie would like to check that Percy knows \(s\) such that \(h=g^s\). Consider the below protocol, due to Schnorr:
\begin{enumerate}
