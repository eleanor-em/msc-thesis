...
\begin{itemize}
    ...
    \item the thesis is 16330 words in length (excluding text in images, tables, bibliographies, and appendices).\EMNote{todo}
\end{itemize}
\newpage
...
\begin{proof}
    ...
    By outputting the same bit as $\mathcal{A}$, $\mathcal{B}$ has advantage at most $\varepsilon$. Since the DDH assumption holds in $\mathbb{G}$, we must have $\varepsilon=\negl(\lambda)$, so the ElGamal cryptosystem is IND-CPA secure.
\end{proof}
...
We also generate public parameters $\mathcal{P}=(G, H, P)$ of a Pedersen commitment scheme generated such that nobody knows $\mathsf{dlog}_G{H}$, \eg by choosing elements according to a hash function; see Section~\ref{sec-pedersen} for details.
