...
Many existing systems (based on a principle called \textit{code voting}) suffer from a major drawback: the \textbf{integrity} of the system depends on the \textbf{secrecy} of the codes. In principle, this secrecy is impossible to verify---printing and sending these codes in secret is a major practical obstacle for such schemes, so the integrity because \VTNote{becomes.  This is really good, Eleanor.  My only comment is that there's a bit of a non sequitur re code voting - you probably want a sentence explaining the basic idea before you say that the integrity of the system depends on the secrecy of the codes.  I think the first suggestion is by Chaum: D. Chaum. SureVote: Technical Overview. Proceedings of the Workshop on Trust-worthy Elections. Except that I can't actually find a decent online link.  This slide deck is the best I can do. \url{https://www.yumpu.com/en/document/read/8992700/surevote-technical-overview-iavoss}} difficult or impossible to verify, defeating the purpose. We propose a solution to this problem by having the voter rely on a secrecy assumption, but having the voter generate their own secret. The (untrusted) electoral commission cannot access these secrets without colluding with the voter's device.

The existing work closest to our setting is Verifiable Postal Voting~\cite{benaloh2013verifiable}. We borrow the essential idea of cryptographically-augmented postal voting, with several key improvements: we introduce receipt freeness (for an honest-but-curious voter), we have a much higher probability of detecting attempted fraud (we fail only with negligible probability), and perhaps most importantly we do not rely on an existing public key infrastructure to allow secret communication to the voter.\VTNote{I think it was just for signatures.}
...
\VTNote{Another important collection of related work is the schemes that use a combination of plain paper and crypto verification.  All the prior works are polling-place schemes, but they still deserve a mention, specifically: Scantegrity II (\url{https://www.usenix.org/legacy/event/evt08/tech/full_papers/chaum/chaum_html/}) which is basically code voting using a plain paper ballot), StarVote (\url{https://arxiv.org/pdf/1211.1904.pdf} which is basically Helios plus a plain paper ballot) and Microsoft's ElectionGuard, which is basically the same as StarVote.}
...
\VTNote{It's probably easiest to describe two algorithms: Prove and Verify, then the def's below become a bit easier.}

Zero-knowledge proofs rely on three key properties:
\begin{enumerate}
    \item \textit{completeness}: if the prover honestly knows the secret, they must be able to construct a valid proof.\VTNote{This is usually made a bit clearer by saying 'construct a proof that passes Verify().' so you don't have to (otherwise) define what a valid proof is.}
    \item \textit{soundness}: a prover who does not know the secret must not be able to construct a valid proof, except with negligible probability \VTNote{Again, mustn't be able to pass Verify() except with negl prob.}
    \item \textit{zero-knowledge}: an eavesdropper must learn nothing apart from that the prover knows a secret satisfying the required conditions \VTNote{I'd say that the Verifier must learn nothing.  (And you know there's a subtlety about whether it's honest-verifier or not, which is a fine thing to leave out of here...)}
\end{enumerate}
We will also typically require our zero-knowledge proofs are \textit{universally verifiable}, meaning that any observer (e.g. a voter) can verify the correctness of the proofs.\VTNote{I would shift this sentence about universal verifiability into the section in which you introduce the Fiat-Shamir transform.  The interactive version of these proofs can't be UV because an observer can't tell after the fact whether the verifier secretly shared the challenge in advance.  So you might as well not mention it until you explain how to make them noninteractive.} We formulate a zero-knowledge proof as a protocol where the prover and verifier exchange messages, and the verifier either accepts or rejects the proof. The prover and verifier may either be honest (meaning they correctly follow the protocol), or dishonest (meaning they deviate from the protocol). The treatment that follows is based on that of~\cite{boneh2020graduate}.
...
The simplest proof we will use dates back to Schnorr~\cite{schnorr1991efficient}. The prover $\mathcal{P}$ publishes a group $\mathbb{G}$, a base element $g\in\mathbb{G}$, and another element $y = g^x$ for some secret $x$. $\mathcal{P}$ produces a proof that it knows the value of $x$ as follows:
 ...
 \VTNote{You prob want $\mathbb{G}$ input to the hash too, to stop people pretending it was all set in some other group.}
...
A proof of knowledge for discrete logarithms can be readily extended to proving plaintext knowledge for ElGamal encryption: assuming that $(a, b) = (g^r, my^r)$ is a genuine ciphertext, then proving knowledge of $r$ suffices to prove knowledge of $m$ since the prover could simply raise $y$ to that power and divide to recover $m$. To be an adaptively sound proof, the challenge hash must now include both elements of the ciphertext; see~\cite{mcmurtry2020test} for examples of what can go wrong if we do not do so.  \VTNote{Write `The algorithm is given in ref...' Because your algorithms are floats, they're showing up in slightly odd places.}
...
Another useful extension of this idea allows a prover to demonstrate knowledge of $x$ such that $u=g^x$ and $v=h^x$, or in other words that $\dlog_g u=\dlog_h v$. The proof essentially works by running two instances of Protocol 1 simultaneously.\VTNote{The algorithm is shown in ref...}
...
Another application of $\mathsf{PrfEqDlogs}$ allows the prover (holding the secret key) to demonstrate whether two ciphertexts are encryptions of the same message. The idea will be to divide the ciphertexts $(a_1, b_1), (a_2, b_2)$ element-wise, re-randomise the result to obscure any information about the plaintext, and decrypt the re-randomisation. \VTNote{Strictly speaking it's not just rerandomisation because it also raises the message to a random power, which is important for obscuring the true ratio of the input messages.} If the resulting message is 1, the plaintexts are equal except with negligible probability.\footnote{If we are very unlucky (or the prover is malicious), the randomisation factor may be the order of the group, resulting in the identity ciphertext $(1, 1)$ which will always be an encryption of 1. See~\cite{mcmurtry2020test} for details.} Once again we must modify the challenge hash to include all data.
...
\VTNote{I'm not sure you even need a summary.  It's really very clear.}
