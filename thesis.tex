...
Remote voting is on the rise worldwide, either in the form of online or postal voting. Online voting can often suffer from unreasonable trust assumptions and verifiability issues that do not arise from postal voting. However, the latter is difficult to administrate;\VTNote{administer} current postal voting systems rely on a send-and-return model that introduces long delays and opportunities for fraud. We aim to investigate how an electronically-generated ballot can allow one-way postal voting, and how a cryptographic protocol can produce integrity and privacy guarantees. We will analyse the trust assumptions needed for such a system,\VTNote{I'd stick the trust assumptions with the 'investigate how...' rather than 'analyse ... needed' part because we don't really do any nontrivial analysis of what is necessary, just what our protocol relies on.} demonstrate desirable privacy and verifiability properties of the protocol, and provide a prototype implementation.\VTNote{plus prove that it satisfies them.}
...
The key innovation of the protocol involves pairing votes with a \textit{message authentication code} (MAC) constructed as a function of the vote and secret parameters chosen when the vote is generated. The secret parameters define a line, so if the vote and MAC are known, there are still a large number of possible values for the secrets. This makes it infeasible for an adversary who does not know the secrets to generate a \VTNote{different} valid vote-MAC pair, providing strong integrity guarantees.
...
\begin{itemize}
    ...
    \item \textit{tallied-as-cast verifiability}: everybody should be confident that every cast vote was counted correctly in the final tally \VTNote{I'd say every received vote or every recorded vote - the point being that 'everybody' can only look at what was recorded/received, not what was meant to be cast.}
\end{itemize}
...
\VTNote{Security proofs?  Code or implementation details?  You might like to mention at this point where they are or how to access them, even if they're not pasted in to the thesis.}
...
\end{itemize}
\VTNote{You probably also want to mention that you mostly leave out the 'mod p' part of modular arithmetic equivalences.  } 
...
\VTNote{In this def, define p.  Also, I'd just say the secret key is $x$, but you could also say the secret key is all that plus x, in which case you need to include p.  I'd also make it a little clearer to the uninitiated that $r$ is newly generated each time, i.e. say something like ``encryption is computed by generating a randomisation factor r and then...''} 
...
\end{definition}
It is not immediately clear that such a group, its operations, and an appropriate $g$ can be efficiently computed. Appendix~\ref{app-elgamal} expands on the relevant mathematical details.  \VTNote{I would say for the semi-initiated that in this thesis you always use the exponential version in which $m = g^M$ for some real message $M$.  Mention that this is a slightly-nonstandard form, i.e. the exponential form, but that it allows for homomorphic addition - indeed, homomorphic addition might warrant Sec 2.2.4, explaining how you can add and do scalar multiplication, because we use that later (right?).  Mention that it only works for efficiently-enumerable $M$.}
...
Note that any additively homomorphic scheme supporting proof constructions would suffice. One other such example is the Pallier cryptosystem; we choose the ElGamal system instead for the sake of simplicity.\VTNote{Equivalent proof contstructions and distributed key generation and decryption.  These are a fair bit harder for Paillier because it's dependent on a composite modulus with secret prime factors - much harder to do distributed decryption.  So probably the more plausible other candidates are not Paillier but other dlog systems.  btw, you actually implemented it in an EC system didn't you?  It might therefore be better to use this comment to mention that it all bascially just works in an elliptic curve in which the DDH problem is hard.  Leave Paillier out of it.  Come to think of it, you should probably mention that the security of El Gamal is dependent on DDH, possibly in Sec 2.2.2.}
...
\VTNote{I would actually describe this a little differently, as a commitment to some particular value $a$, which need not be randomly/uniformly chosen, using a blinding factor (though you don't have to call it that) which has to be randomly and uniformly chosen from $\mathbb{Z}_P$.  This relates to the def of perfectly hiding below}
...
, these are the same values they originally committed to. 
\VTNote{The concept here is \emph{computationally binding:} Assuming that the Dlog of H wrt G is hard to compute, a polynomially-bounded prover has negligible probability of opening a commitment two different ways.  The reduction is actually pretty easy: start with two different openings of the same commitment, and convince yourself you can infer the Dlog of H base G  (or maybe of G base H).}
Because the commitment function is many-to-one, this scheme is \textit{perfectly-hiding}: from an information-theoretic perspective, the commitment reveals nothing about the values of $a$ and $b$. \VTNote{Perfectly hiding is actually even stronger than being many-to-one: any given commitment c is equally likely to have been produced as a commitment to any a, because for each a there's exactly one b that produces c.  Hence c reveals no information.  Note you have to be careful with the orders of G,H - I think this argument is only valid if their order is prime.}
