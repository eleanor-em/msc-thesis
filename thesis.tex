...
\begin{proof}
    ...
    \end{itemize}
    In Game $G_4$, the encryptions are random and contain no usable information, so the adversary cannot have any advantage in guessing which vote was cast by each voter without violating condition 2. Therefore, $\Adv(\mathcal{A}, G_3)=0$, which implies $\Adv\left(\mathcal{A}, G_{\text{priv}}(1^\lambda, n, m)\right)=\negl(\lambda)$ as required.
\end{proof}
